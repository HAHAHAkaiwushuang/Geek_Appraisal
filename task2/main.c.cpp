...
int main(int argc,char *argv[])
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)
        {
            arr_small[count2++] = arr_big[i];	//把数组arr_big统计的字母个数转化到数组arr_small中进行统计
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*判断该节点是否为叶节点*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*该结点还未赋值，无法进入循环比较，故退出循环不比较*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*不断比较结点的权重大小，直到输出一个权重最小的结点*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
